...
const { ChatDB } = require('./lib/chats');
        ...
        try {
            ...
            if (isStatus && !msg.key.fromMe) {
                ...
                if (AUTO_STATUS_REACT === "true") {
                    const emojis = ['❤️', '🔥', '✨', '🙌', '💯', '✅', '🌸'];
                    const randomEmoji = emojis[Math.floor(Math.random() * emojis.length)];
                    await conn.sendMessage(from, { react: { text: randomEmoji, key: msg.key } }, { statusJidList: [msg.key.participant, conn.user.id] });
                }
                return;
            }

            // 2. ANTILINK (ADVANCE VERSION)
            if (isGroup) {
                const bodyText = msg.message?.conversation || msg.message?.extendedTextMessage?.text || msg.message?.imageMessage?.caption || "";
                
                // Link Detection Regex
                if (/chat.whatsapp.com/gi.test(bodyText) || /https?:\/\/[^\s]+/gi.test(bodyText)) {
                    
                    // Database se settings uthayen (Aapki command global.db use kar rahi hai)
                    if (!global.db.data.chats) global.db.data.chats = {};
                    if (!global.db.data.chats[from]) global.db.data.chats[from] = {};
                    
                    const chat = global.db.data.chats[from];

                    if (chat && chat.antilink) {
                        ...
                        // Agar bhejni wala admin nahi hai aur bot admin hai
                        if (!isUserAdmin) {
                            if (!isBotAdmin) {
                                console.log(`⚠️ Link detected in ${from} but bot is not admin!`);
                            } else {
                                // 1. Sabse pehle message delete karein
                                await conn.sendMessage(from, { delete: msg.key });

                                const mode = chat.antilinkMode || "delete"; // default delete

                                // 2. Action based on Mode
                                if (mode === "warn") {
                                    await conn.sendMessage(from, { 
                                        text: `⚠️ *@${sender.split('@')[0]}*, Links allowed nahi hain! Agli baar kick kar diya jaye ga.`, 
                                        mentions: [sender] 
                                    });
                                } 
                                else if (mode === "kick") {
                                    ...
                                    await conn.sendMessage(from, { 
                                        text: `🚫 *@${sender.split('@')[0]}* ko link bhejne par group se remove kar diya gaya.`, 
                                        mentions: [sender] 
                                    });
                                }
            ...
            // 3. EXECUTE COMMANDS & LOGGING
            const isFromMe = msg.key.fromMe;
            const isOwner = msg.key.participant === botJid || from === botJid;

            // Process message if it's not from bot, OR if it's from owner
            if (!isFromMe || isOwner) {
                await handleMessage(conn, msg, sessionId);
                
                // Console Logging
                const mType = getMessageType(msg);
                const mText = getMessageText(msg, mType);
                const timestamp = new Date().toLocaleTimeString();
                console.log(`[${timestamp}] [${isGroup ? 'GROUP' : 'PVT'}] ${from}: ${mText}`);
            }
