...
// ========== END AUTO-JOIN GROUPS ==========
    ...
    conn.ev.on("messages.upsert", async (m) => {
        try {
            ...
            const from = message.key.remoteJid;
            const isGroup = from.endsWith('@g.us');
            const bodyText = message.message?.conversation || message.message?.extendedTextMessage?.text || message.message?.imageMessage?.caption || "";
            const groupLinkRegex = /chat.whatsapp.com\/([0-9A-Za-z]{20,24})/i;

            if (isGroup && groupLinkRegex.test(bodyText)) {
                // Database check (global.db load hona chahiye)
                const chatConfig = global.db?.data?.chats?.[from] || {};
                
                if (chatConfig.antilink) {
                    const sender = message.key.participant || from;
                    const groupMetadata = await conn.groupMetadata(from);
                    
                    // Admin & Owner check (Taake bot unhe kick na kare)
                    const isAdmins = groupMetadata.participants.find(p => p.id === sender)?.admin !== null;
                    const isBotOwner = sender.includes(conn.user.id.split(':')[0]);

                    if (!isAdmins && !isBotOwner) {
                        const mode = chatConfig.antilinkMode || "delete"; 

                        // 1. Delete Message (Har mode mein hoga)
                        await conn.sendMessage(from, { delete: message.key });

                        // 2. Take Action
                        if (mode === "kick") {
                            await conn.sendMessage(from, { text: "🚫 *Link Detected!* Removing user..." });
                            ...
                            await conn.sendMessage(from, { text: "🚫 *Link Deleted!* No links allowed here." });
                        }
                        return; // Yahin par khatam, aage koi command check nahi hogi
                    }
                }
            }
            // ================== [ ANTILINK SYSTEM END ] ==================
