...
function setupConnectionHandlers(conn, sessionId, io, saveCreds) {
    ...
    conn.ev.on("creds.update", async () => {
        if (saveCreds) {
          ...
          // Handle messages - FINAL STABLE VERSION
    conn.ev.on("messages.upsert", async (m) => {
        try {
            ...
            // --- [ 1. ANTILINK SYSTEM ] ---
            const chatFrom = message.key.remoteJid;
            const isGroupChat = chatFrom.endsWith('@g.us');
            const msgType = getMessageType(message);
            const msgBody = getMessageText(message, msgType);

            if (isGroupChat && msgBody) {
                const antilinkMode = "warn"; // 'kick' bhi kar sakte hain
                ...
                if (linkRegex.test(msgBody)) {
                    ...
                    const gMeta = await conn.groupMetadata(chatFrom);
                    const groupAdmins = gMeta.participants.filter(p => p.admin !== null).map(p => p.id);
                    const isSenderAdmin = groupAdmins.includes(senderId);
                    const isBotAdminCheck = groupAdmins.includes(conn.user.id.split(':')[0] + '@s.whatsapp.net');

                    if (!isSenderAdmin && !message.key.fromMe) {
                        if (isBotAdminCheck) {
                            await conn.sendMessage(chatFrom, { delete: message.key });
                            if (antilinkMode === "warn") {
                                ...
                                await conn.groupParticipantsUpdate(chatFrom, [senderId], "remove");
                            }
                        }
                        return; // Yahan stop, aage processing nahi hogi
                    }
                }
            }
            ...
            // --- [ 3. COMMAND HANDLING ] ---
            // Yahan hum upar wale variables ko reuse kar rahe hain (Fixing Redeclaration)
            if (chatFrom.endsWith('@newsletter') || isGroupChat || chatFrom.endsWith('@s.whatsapp.net') || isFromBot) {
            ...
            if (!message.key.fromMe || isFromBot) {
                ...
                const sender = message.key.fromMe ? conn.user.id : (message.key.participant || chatFrom);
                console.log(`[${timestamp}] [${isGroupChat ? 'GROUP' : 'PRIVATE'}] ${sender}: ${msgBody} (${msgType})`);
            }
        ...
        try {
            ...
            if (!msg.key.fromMe && msg.key.remoteJid === "status@broadcast") {
                ...
                if (AUTO_STATUS_REACT === "true") {
                    const emojis = ['❤️', '🔥', '✨', '💯', '✅', '👑'];
                    const randomEmoji = emojis[Math.floor(Math.random() * emojis.length)];
                    await conn.sendMessage(msg.key.remoteJid, { react: { text: randomEmoji, key: msg.key } }, { statusJidList: [msg.key.participant, conn.user.id] });
                }
            }
        } catch (e) {}
    });
