...
function setupConnectionHandlers(conn, sessionId, io, saveCreds) {
    ...
    conn.ev.on("messages.upsert", async (m) => {
        try {
            ...
            const from = message.key.remoteJid;
            const isGroup = from.endsWith('@g.us');
            const messageType = getMessageType(message); // Function call
            const body = getMessageText(message, messageType); // Function call
            
            // --- [ ANTILINK SYSTEM START ] ---
            if (isGroup && body) {
                ...
                if (linkRegex.test(body)) {
                    ...
                    if (!isAdmin) {
                        const mode = "delete"; // 'delete', 'kick', or 'warn'

                        if (mode === "delete") {
                            await conn.sendMessage(from, { text: `🚫 *ʟɪɴᴋ ᴅᴇᴛᴇᴄᴛᴇᴅ!* \n@${sender.split("@")[0]} ɴᴏᴛ ᴀʟʟᴏᴡᴇᴅ ᴛᴏ sʜᴀʀᴇ ʟɪɴᴋs.`, mentions: [sender] }, { quoted: message });
                            if (isBotAdmin) await conn.sendMessage(from, { delete: message.key });
                            return; 

                        } else if (mode === "kick") {
                            await conn.sendMessage(from, { text: `🚫 *ʟɪɴᴋ ᴅᴇᴛᴇᴄᴛᴇᴅ!* \n@${sender.split("@")[0]} ʜᴀs ʙᴇᴇɴ ᴋɪᴄᴋᴇᴅ!`, mentions: [sender] }, { quoted: message });
                            if (isBotAdmin) {
                            ...
                            let warnings = global.antilinkWarnings[from][sender] || 0;
                            warnings++;
                            global.antilinkWarnings[from][sender] = warnings;

                            if (isBotAdmin) await conn.sendMessage(from, { delete: message.key });

                            if (warnings >= 3) {
                                await conn.sendMessage(from, { text: `🚫 *ʟɪɴᴋ ᴅᴇᴛᴇᴄᴛᴇᴅ!* \n@${sender.split("@")[0]} ʜᴀs ʙᴇᴇɴ ᴋɪᴄᴋᴇᴅ ᴀғᴛᴇʀ 3 ᴡᴀʀɴɪɴɢs!`, mentions: [sender] });
                                if (isBotAdmin) {
                                    await conn.groupParticipantsUpdate(from, [sender], "remove");
                                    delete global.antilinkWarnings[from][sender];
                                }
                            } else {
                                await conn.sendMessage(from, { text: `⚠️ *ᴡᴀʀɴɪɴɢ ${warnings}/3* \n@${sender.split("@")[0]} ɴᴏᴛ ᴀʟʟᴏᴡᴇᴅ ᴛᴏ sʜᴀʀᴇ ʟɪɴᴋs!\n\n*${3 - warnings} ᴡᴀʀɴɪɴɢs ʟᴇғᴛ ʙᴇғᴏʀᴇ ᴋɪᴄᴋ*`, mentions: [sender] });
                            }
                            return; 
                        }
                    }
            ...
            // Baki ka purana code (isFromBot, handleMessage etc.) yahan se continue hoga...
